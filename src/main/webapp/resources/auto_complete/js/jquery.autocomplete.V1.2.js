...
$.Autocompleter = function(input, options) {
	...
	function hideResultsNow(term) {//增加参数term ，以便于判断是否是没有找到匹配结果。add by rwz
				...
				function (result){
					...
					if( !result ) {
						if (options.multiple) {
							...
							$input.val( "" );
							if (typeof(term)!="undefined"){//rwz modify 使其支持  无匹配提示 
								$input.trigger("result", options.noResultTip);
							} else {
...
$.Autocompleter.defaults = {
	...
	noResultTip:"未找到匹配的结果",//rwz 无匹配提示
	max: 100,
	mustMatch: false,
	extraParams: {},
	selectFirst: true,
	...
	return {
		...
		load: function(q) {
			...
			if( !options.url && options.matchContains ){
				...
				// track all matches
				var csub = [];
				//为了保证自动补全数据的顺序不被改变，这里做了一些调整，不再使用原先的优化部分。当且仅当 现有配置下有效： 传入数据而非url + matchContains==true
				/* rwz hide  原始部分
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	return {
		...
		show: function() {
			...
			var widthfix = parseInt($(input).css('padding-left')) + parseInt($(input).css('padding-right')) +  parseInt($(input).css('border'))*2+ 1;//宽度修正 by rwz 暂时没有考虑margin
			element.css({
				width: typeof options.width == "string" || options.width > 0 ? options.width : $(input).width() + widthfix,

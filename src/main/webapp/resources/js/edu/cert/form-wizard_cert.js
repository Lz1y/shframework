		...
		init : function () {
			...
			var handleTitle = function(tab, navigation, index) {
				...
				if (current == total - 1) {
					...
					$('#form_wizard_1').find('.button-next').append($('<i class="m-icon-swapright m-icon-white"></i>'));
			   	 	...
			   	 	$('#form_wizard_1').find('.button-next').attr("class", "btn blue button-next");
			...
			$('#form_wizard_1').bootstrapWizard({
				...
				onNext : function (tab, navigation, index) {
					...
					//导入
					if(index==2){
						  var srcurl = getContextPath()+"/edu/cert/certimport/0/0/import/directaccess";//真正导入时的请求
							var param = {importType:$("input[name='importType']:checked").val()};
						  jQuery.ajax({
  								url : srcurl,
  								method: "POST",
  								async:false, 
					    		...
					    		success : function(data,textStatus,jqXHR ) {
					    			...
					    			if (data.sucmsg!=null){
					    				   ...
					    				   htmlText =  "<strong>" + data.tip + "</strong>";

 ...
 */
function autocomplete_course_init_ajax(nameSelector, idSelector){
	var data = {};
	$.ajax({
    	url: getContextPath() + '/edu/skd/course/0/0/dict/directaccess',
      type: "POST",
      async:true, 
    	data: data,
    	dataType: 'json',
    	success: function(result) {
    		if (typeof(result.data) != 'undefined'){
    			...
    			init_autocomplete_course(nameSelector, idSelector, jsonObj);
    		}
    	}
    });
}

function init_autocomplete_course(nameSelector, idSelector, jsonObj){
			...
			$name.result(function(event, data, formatted){//当某一项被选中时/没有符合条件的结果时
				if(typeof(data)=="string"){//没有结果时--可能是由于没有找到匹配的结果，或者没有选中任何东西
					...
					var result = data.title + "(" + data.code + ")";

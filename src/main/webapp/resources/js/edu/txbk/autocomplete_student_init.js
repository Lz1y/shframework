 ...
 */
function autocomplete_student_init_ajax(nameSelector, idSelector, acdYearTermId){
	var data = {};
	$.ajax({
      url: getContextPath() + '/edu/txbk/settlementarrearage/0/0/student/directaccess?acdYearTermId=' + acdYearTermId,
      type: "POST",
      async:true, 
    	data: data,
    	dataType: 'json',
...
function init_autocomplete_entry(nameSelector, idSelector, jsonObj){
			...
			$name.autocomplete(jsonObj, {
					...
					formatItem: function(row, i, max, item) {//格式化列表中的条目
						//
						var result = row.studentName + "(" + row.studentNo + ")";
						return  result;
					},
					...
					formatResult: function(row) {//被选中时输出的内容
						return row.studentName + "(" + row.studentNo + ")";
					}

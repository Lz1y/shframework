 ...
 */
function autocomplete_textbook_init_ajax(nameSelector, idSelector){
  var additionalOrderId = $("input[name='additionalOrderId']").val();
	var data = {additionalOrderId:additionalOrderId};
	$.ajax({
    	url: getContextPath() + '/edu/txbk/additionalorder/0/0/textbook/directaccess',
      type: "POST",
      async:true, 
    	data: data,
    	dataType: 'json',
...
function init_autocomplete_entry(nameSelector, idSelector, jsonObj){
			...
			$name.autocomplete(jsonObj, {
					...
					formatItem: function(row, i, max, item) {//格式化列表中的条目
						var result = row.textbookName + "(" + row.isbn + "-单价" + row.price + ")";
						return  result;
					},
					...
					formatResult: function(row) {//被选中时输出的内容
						return row.textbookName + "(" + row.isbn + "-单价" + row.price + ")";
					}
			...
			$name.result(function(event, data, formatted){//当某一项被选中时/没有符合条件的结果时
				if(typeof(data)=="string"){//没有结果时--可能是由于没有找到匹配的结果，或者没有选中任何东西
					...
					$(idSelector).val(data.textbookId);
				}
			});//result

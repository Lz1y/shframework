var FormImageCrop = function () {
	...
	var demo9 = function() {
		...
		$('#modal_img').Jcrop({
			...
			// Store the API in the jcrop_api variable
			jcrop_api = this;
			jcrop_api.setOptions({ boxWidth: '1024px',boxHeight:'768px' });
			// Move the preview into the jcrop container for css positioning
			$preview.appendTo(jcrop_api.ui.holder);
		});
	...
	var demo10 = function() {
		...
		$('#modal_img').Jcrop({
			...
			jcrop_api = this;
			// Move the preview into the jcrop container for css positioning
			// jcrop_api.animateTo([10, 10, 140, parseInt(50*(xsize / ysize)) + 10]);
			$preview.appendTo(jcrop_api.ui.holder);
		});
		...
		function updatePreview(c){
			...
			if (parseInt(c.w) > 0) {
				...
				console.log('x -> ' + Math.round(c.x));
				console.log('y -> ' + Math.round(c.y));
				console.log('w -> ' + Math.round(c.w));
				console.log('h -> ' + Math.round(c.h));
				var bounds = this.getBounds();
				boundx = bounds[0];
				boundy = bounds[1];
				$pimg.css({

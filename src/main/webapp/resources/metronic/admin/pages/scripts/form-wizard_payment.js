			...
			$('#form_wizard_1').bootstrapWizard({
				...
				onNext : function (tab, navigation, index) {
					...
					if(index==4){
						  ...
						  var scope = $("#scope").val();
						  var srcurl = contextPath+"/edu/roll/payment/"+scope+"/-1/import";
						  
						  var templateId= $("#templateId").val();
						  var fileFormat= $("input[name='file_format']:checked").val();
						  
						  var radioFee= $("input[name='fee']:checked").val();
						  var feeThreshold = $("#feeThreshold").val();
							
	            if(radioFee==$("#judgeByFee").val()){
	            	if(feeThreshold==""){
	            		 bootbox.alert("实收金额单选框，请输入金额!");
                    return false;
	            	}
	            }
	            var arrThreshold = $("#arrThreshold").val();
	            if(radioFee==$("#judgeByArr").val()){
	            	if(arrThreshold==""){
	            		 bootbox.alert("欠费金额单选框，请输入金额!");
                    return false;
	            	}
	            }
					   var param = {
							   ...
							   radioFee:radioFee,
							   feeRole:$("select[name='feeRole'] option:selected").val(),
							   feeThreshold:feeThreshold,
							   arrRole:$("select[name='arrRole'] option:selected").val(),
							   arrThreshold:arrThreshold,
							   isCover:$("input[name='isCover']:checked").val(),
							   matchingResult:$("#select2_sample5").val()
			        };
						  jQuery.ajax({
								url : srcurl,
								method: "POST",
				    		...
				    		success : function(data,textStatus,jqXHR ) {
				    			...
				    			var htmlStr = "";
  								$.each(returnList,function(i,value){
  									htmlStr += "<strong>" + value + "</strong></br>" ;
  								});
				    			$( "#returnResult" ).html( htmlStr );
				    		},
							 error : function( jqXHR,  textStatus,  errorThrown) {
								 $( "#returnResult" ).html( "<strong>导入数据有误，请再试一试</strong>" );
				    		}
				    	});	
					}
					success.hide();

							...
							<div class="row">
								<div class="col-md-8">
										<div class="form-group">
											...
											<div class="col-md-10">
												<@commonMacros.multiTextarea "add" "description" "" "illchar" 'rangelength="0,1024"' />
											</div>
										</div>
									</div>
							</div>
								...
								<div class="form-actions">
									<div class="row">
										<div class="col-md-offset-3 col-md-9">
											<a class="btn green-seagreen" href="javascript:;" onclick="saveDbfExportTemplate();">保存</a>
		...
		<jsinner>
			<script>
				$(document).ready(function() {
					...
					sel.dst.dataKey = 'id';
					sel.dst.title = '已选';
				//	var input_html = '<span style="color:red">*&nbsp;</span><input type="text" name="dbfFileName" size="10" class="textbox required nameFormat" rangelength="2,10" />';
	/*				for(var i = 0 ; i < dataList.length ; i++){
						var hidden2_value = dataList[i].colDbName;
						var input_html2 = "<input type='hidden' name='hidden2Value' size='10'  class='textbox' value='" + hidden2_value +"'/>";
						sel.src.add({id:i+1, colFileName: dataList[i].colFileName,input: input_html,hidden2:input_html2});
					} */
				...
				function saveDbfExportTemplate(){
							...
							$(dbfFileNameArr).each(function(index,item){  
							    dbfFileNameData.push(item.value);  
							}); 
				            
				            var hidden2ValueArr = $("input[name='hidden2Value']");  
							var hidden2ValueData=new Array();  
							var info = "";
							
							$(hidden2ValueArr).each(function(index,item){  
							    hidden2ValueData.push(item.value);  
							     // 如果i+1等于选项长度则取值后添加空字符串，否则为逗号
     							info = (info + item.value) + (((index + 1)== hidden2ValueArr.length) ? '':';'); 
     
							}); 
							//hidden2ValueData.toString() 之后 传递到后台 格式为： [_ernc, campus_id, 校区, _ersr, department_id, 院系]
						   ...
						   var url = "${contextPath}/${curMenu.pUrl}/0/tplsave?module=${curMenu.rule}&pUrl=${curMenu.pUrl}";
						  jQuery.ajax({
								url : url,
								method: "POST",
								async:false, 

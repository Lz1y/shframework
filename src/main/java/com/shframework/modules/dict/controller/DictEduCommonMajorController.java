...
@Controller
@RequestMapping("dict/edu/common/major/{scope}/{scopeId}")
public class DictEduCommonMajorController extends BaseComponent {
	...
	public String showForm(
		...
		PageTerminal<DictEduCommonMajor> page = dictEduCommonMajorService.findAllByPage(pageSupport, false);
		model.addAttribute("page", page);
		return  "/dict/major/dict_major_list";
	}
	...
	public String detail( 
		...
		DictEduCommonMajor dictMajor = dictEduCommonMajorService.getDict(id, false);
		model.addAttribute(DICT_ENTITY_NAME, dictMajor);
		model.addAttribute("pageSupport", pageSupport);
		return "/dict/major/dict_major_detail";
	}
	...
	public String add(Model model, @DictParam PageSupport pageSupport){
		...
		model.addAttribute("pageSupport", pageSupport);
		return  "/dict/major/dict_major_form";
	}
	...
	public String edit( 
		...
		if (StringUtils.equals(type, Constants.DICT_EDIT_TYPE_CHANGESTATUS)){
			...
			return "redirect:/dict/edu/common/major/" + scope + "/" + scopeId + "/all/list" ;
		}		
		if(!model.containsAttribute(DICT_ENTITY_NAME)) {
			model.addAttribute(DICT_ENTITY_NAME, dictEduCommonMajorService.getDict(id, false));
		}
		model.addAttribute("pageSupport", pageSupport);
		return "/dict/major/dict_major_form";
	}
	...
	public String save(
		...
		return "redirect:/dict/edu/common/major/" + scope + "/" + scopeId + "/0/add";
	}
	...
	public String update(
		...
		if (!result.hasErrors()) {
			...
			if (saveResult >= 0){
				redirectAttributes.mergeAttributes(pageSupport.getParamVo().getParmMap());
				return "redirect:/dict/edu/common/major/" + scope + "/" + scopeId + "/all/list" + pageSupport.getParamVo().getParm();
			} 
			else if (saveResult == Constants.ERR_SAVE_DUPLICATEKEY){
				result.rejectValue("errorMsg", "error.duplicatekey", "");
			}
		...
		return "redirect:/dict/edu/common/major/" + scope + "/" + scopeId + "/" + dictId + "/edit";
	}
	...
	public String delete(
			...
			RedirectAttributes redirectAttributes,
			@DictParam PageSupport pageSupport){
		int result = dictEduCommonMajorService.deleteById( id, false);
		if (result == -1){
			redirectAttributes.addFlashAttribute("errTips", "请先删除关联该专业的校定专业");
		}
		redirectAttributes.mergeAttributes(pageSupport.getParamVo().getParmMap());
		return "redirect:/dict/edu/common/major/" + scope + "/" + scopeId + "/all/list";
	}
